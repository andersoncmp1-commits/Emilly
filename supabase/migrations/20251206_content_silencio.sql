-- Migration to add content for "Silêncio na Missa e na Igreja" module
...
BEGIN
  ...
  IF v_module_id IS NOT NULL THEN
    ...
    -- Create Section 1: A Espiritualidade do Silêncio
    INSERT INTO sections (module_id, title, position)
    VALUES (v_module_id, 'A Espiritualidade do Silêncio', 0)
    RETURNING id INTO v_section_id_1;

    -- Lesson 1.1: Característica da Missa
    INSERT INTO lessons (section_id, title, content, type, position)
    VALUES (v_section_id_1, 'Uma Característica da Missa Tridentina', 
    ...
    <p>Além disso, sabemos que a mortificação é indispensável para uma <strong>vida espiritual ordenada</strong>. O silêncio ajuda bastante nesse ponto: negação de si, a mortificação de si.</p>', 'text', 1);

    -- Lesson 1.3: Incentivo à Meditação
    INSERT INTO lessons (section_id, title, content, type, position)
    VALUES (v_section_id_1, 'Incentivo à Meditação', 
    ...
    <p>O <strong>silêncio</strong>, a orientação do padre, voltado para <strong>Deus</strong>, e o latim nos ajudam bastante a <strong>servir a um só Senhor – a Santíssima Trindade – e a buscar em primeiro lugar o reino de Deus</strong>.</p>', 'text', 3);


    -- Create Section 2: Normas Práticas
    INSERT INTO sections (module_id, title, position)
    VALUES (v_module_id, 'Normas Práticas', 1)
    RETURNING id INTO v_section_id_2;

    -- Lesson 2.1: Guardado na Igreja
    INSERT INTO lessons (section_id, title, content, type, position)
    VALUES (v_section_id_2, 'A Guarda do Silêncio', 
